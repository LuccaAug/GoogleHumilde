...
TEST_SUITE("Dataset"){
	...
	TEST_CASE("Palavra_indice()"){
		...
		CHECK(d.Palavra_indice("pds") == 2);
		CHECK(d.Palavra_indice("2") == 3);
	...
	TEST_CASE("Quantos_documentos_possuem_essa_palavra()"){
		...
		int idAbsurdo = d.Palavra_indice("absurdo");
	...
	TEST_CASE("Quantas_vezes_dj_possui_Px()"){
		Dataset d("../documentos/testes/");
		d.Le_lista();
		CHECK(d.Quantas_vezes_dj_possui_Px(1,0) == 1); // A.txt - documento
		CHECK(d.Quantas_vezes_dj_possui_Px(1,1) == 1); // A.txt - teste
		CHECK(d.Quantas_vezes_dj_possui_Px(1,2) == 2); // A.txt - pds
		CHECK(d.Quantas_vezes_dj_possui_Px(1,3) == 0); // A.txt - 2
		CHECK(d.Quantas_vezes_dj_possui_Px(2,0) == 1); // B.txt - documento
		CHECK(d.Quantas_vezes_dj_possui_Px(2,1) == 2); // B.txt - teste
		CHECK(d.Quantas_vezes_dj_possui_Px(2,2) == 0); // B.txt - pds
		CHECK(d.Quantas_vezes_dj_possui_Px(2,3) == 1); // B.txt - 2
